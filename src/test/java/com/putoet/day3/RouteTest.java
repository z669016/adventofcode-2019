...
class RouteTest {
    ...
    public void testRoute() {
        ...
        assertEquals(6, route1.intersect(route2).stream().min(Comparator.comparing(Point::manhattanDistance)).orElseThrow().manhattanDistance());

        route1 = new Route(List.of("R75", "D30", "R83", "U83", "L12", "D49", "R71", "U7", "L72"));
        route2 = new Route(List.of("U62", "R66", "U55", "R34", "D71", "R55", "D58", "R83"));
        assertEquals(159, route1.intersect(route2).stream().min(Comparator.comparing(Point::manhattanDistance)).orElseThrow().manhattanDistance());
        ...
        assertEquals(135, route1.intersect(route2).stream().min(Comparator.comparing(Point::manhattanDistance)).orElseThrow().manhattanDistance());
    }
}