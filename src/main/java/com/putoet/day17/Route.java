...
public class Route {
    ...
    private static List<String> splitRoute(List<List<String>> function, List<String> route) {
        final String[] replacement = new String[]{"A", "B", "C"};

        for (int i = 0; i < 3; i++) {
            final List<String> longest = longestRecurring(route);
            if (!longest.isEmpty()) {
    ...
    public static String routeAsString(List<String> route) {
        return String.join(",", route);
    }

    public static List<String> longestRecurring(List<String> route) {
        int len = 10;
        List<String> longestRecurring = recurringOf(len, route);
    ...
    private static List<String> recurringOf(int len, List<String> route) {
        for (int offset = 0; offset < route.size() - len; offset++) {
            final List<String> subList = route.subList(offset, offset + len);
    ...
    public static int indexOf(List<String> list, int offset, List<String> subList) {
        for (int i = offset; i <= list.size() - subList.size(); i++) {
            boolean match = true;
            for (int x = 0; x < subList.size() && match; x++) {
    ...
    public static boolean containsSubList(List<String> list, int offset, List<String> subList) {
        return indexOf(list, offset, subList) != -1;
    }

    public static List<String> replaceSubList(List<String> list, List<String> subList, String replacement) {
        int offset = indexOf(list, 0, subList);
        while (offset != -1) {
